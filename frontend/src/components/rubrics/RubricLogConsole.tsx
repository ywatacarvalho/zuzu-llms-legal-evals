...
import { getRubricLogs } from "@/services/rubricsApi";

interface RubricLogConsoleProps {
  rubricId: string;
  active: boolean;
}
...
export function RubricLogConsole({ rubricId, active }: RubricLogConsoleProps) {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      const { lines: newLines, total } = await getRubricLogs(rubricId, offsetRef.current);
  ...
  return (
    <Card className="p-4">
      ...
      >
        {lines.length === 0 ? (
          <span className="text-muted-foreground">{t("rubrics.detail.logWaiting")}</span>
        ) : (
          lines.map((line, i) => (
